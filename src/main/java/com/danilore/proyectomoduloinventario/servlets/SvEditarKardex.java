 ...
 * @author Danilore
 */
@WebServlet(name = "SvEditarKardex", urlPatterns = {"/SvEditarKardex"})
public class SvEditarKardex extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        Kardex kardex = control.getKardex(id_editar);

        kardex = control.getKardex(id_editar);

        HttpSession misesion = request.getSession();
        misesion.setAttribute("editarIngresoProducto", kardex);
        ...
        if (produ != null) {
            if (tipoMov != null) {
                if (tipo != null) {
                    ...
                    response.sendRedirect("Vistas/editarKardex.jsp?error=tipoMonedaNoExiste");
                }
            } else {
                response.sendRedirect("Vistas/editarKardex.jsp?error=tipomovimientoNoExiste");
            }

        } else {
            response.sendRedirect("Vistas/editarKardex.jsp?error=productoNoExiste");
        }
    }
